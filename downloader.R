...
insert_copied_text_from_API_token <- 'here'
...
claim <- jwt_claim(  iss = service_key$client_id,
...
# because the file structure and folder names is different between the different
# datasets, they are all handled on their own
...
file <- file.path(temp_dir, "file.zip")
GET(url = download_url_2000, write_disk(file, overwrite=T))

# Unzip the file
file <- paste0(dir,"/file.zip")
unzip(file, exdir = temp_dir)
file.remove(file)

file <- paste0(dir,"/Results/EEA.zip")
unzip(file, exdir = temp_dir)
file.remove(file)

file <- paste0(dir,"/U2006_CLC2000_V2020_20u1.zip")
unzip(file, exdir = data_dir)
file.remove(file)

file <- paste0(dir,"/U2006_CLC2000_V2020_20u1_raster100m_tiled_doc.zip")
unzip(file, exdir = doc_dir)
file.remove(file)

unlink(paste0(dir,"/Results"), recursive = TRUE)

unlink(paste0(doc_dir, "/Info/Legend/Vector"), recursive = TRUE)
unlink(paste0(doc_dir, "/Info/Documents/Vector"), recursive = TRUE)

file.copy(from = list.files(paste0(doc_dir, "/Info"), full.names = T, recursive=T), to = doc_dir)
unlink(paste0(doc_dir, "/Info"), recursive=T)
...
# Download the .zip file
file <- file.path(temp_dir, "file.zip")
GET(url = download_url_2006, write_disk(file, overwrite=T))
...
file <- paste0(dir,"/U2012_CLC2006_V2020_20u1.zip")
unzip(file, exdir = data_dir)
file.remove(file)

file <- paste0(dir,"/U2012_CLC2006_V2020_20u1_raster100m_tiled_doc.zip")
unzip(file, exdir = doc_dir)
file.remove(file)
...
# Download the .zip file
file <- file.path(temp_dir, "file.zip")
GET(url = download_url_2012, write_disk(file, overwrite=T))
...
file <- paste0(dir,"/U2018_CLC2012_V2020_20u1_raster100m.zip")
unzip(file, exdir = data_dir)
file.remove(file)

file <- paste0(dir,"/U2018_CLC2012_V2020_20u1_raster100m_tiled_doc.zip")
unzip(file, exdir = doc_dir)
file.remove(file)
...
# Download the .zip file
file <- file.path(temp_dir, "file.zip")
GET(url = download_url_2018, write_disk(file, overwrite=T))
...
file <- paste0(dir,"/U2018_CLC2018_V2020_20u1.zip")
unzip(file, exdir = data_dir)
file.remove(file)

file <- paste0(dir,"/U2018_CLC2018_V2020_20u1_raster100m_tiled_doc.zip")
unzip(file, exdir = doc_dir)
file.remove(file)
...
CLC2012 <- rast("2012/01_Data/U2018_CLC2012_V2020_20u1_raster100m.tif")
CLC2018 <- rast("2018/01_Data/U2018_CLC2018_V2020_20u1.tif")

#check, if script has worked

if(1==1){
  print("Automated download was successfull, you can find your files in the folders called '2000' etc.")
  
}

#yeahh
plot(CLC2000)
plot(CLC2000, xlim=c(-11,5), ylim=c(35,45))

#extract legend, datasetbased README, 